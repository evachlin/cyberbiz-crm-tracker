...
這個東西長怎樣
每天固定時間（預設台北時間 08:00，可自己改）自動連到 Zoho CRM，撈出目前 Stage 還是「公司名單」「本月有機會」或「短期追蹤」的所有交易。
...
短期追蹤：超過 90 個日曆天（不是工作天）標記需優先處理
產出一份靜態網頁（docs/index.html），是一張可依課別／急迫程度篩選、可點欄位排序的表格（不是攤開的巢狀清單），主管直接用瀏覽器網址打開就能看，不需要登入 Cowork 或 Claude。
只顯示 ROSTER 名單裡的人（業務一~四課、POS，加上 Ambrose、Ellie、Sabrina、William），其他 Owner 的交易不會出現。
...
上方篩選鈕可以選「只看某個業務課」或「只看需優先處理」，可複選其中一種篩選條件（課別 + 急迫程度可以同時套用）。
點欄位標題（狀態／交易名稱／課別／業務／工作天數／金額）可以排序，再點一次同一欄會反轉排序方向。
點一列可以展開細節（商品類別、名單來源、預計成交日、備註、Zoho CRM 連結），平常收起來保持表格乾淨。
資料是 Python 算好之後整包塞進網頁的 REPORT_ROWS（一個 JSON 陣列），畫面渲染、排序、篩選都是純前端 JavaScript（scripts/generate_report.py 裡的 REPORT_SCRIPT），一樣不需要任何伺服器。

勾選寄信提醒功能
每列最左邊有勾選框，勾選後按頁面上方「寄送提醒信」，會依業務 email 自動分組，每位業務各開一個 Gmail 網頁版寫信分頁，主旨跟內容已經帶入這位業務被勾選的所有交易名稱、階段、工作天數、狀態，你確認沒問題後自己按送出，不會自動寄出。
